...
class TreePainter extends CustomPainter {
  ...
  void _drawCurvedConnection(
    ...
    if (layout == TreeLayout.bidirectional ||
        direction == TreeDirection.vertical) {
      // Use consistent control point Y to ensure curves converge properly
      final controlY = layout == TreeLayout.bidirectional
          ? (start.dy > end.dy
              ? end.dy + (start.dy - end.dy) * 0.5
  ...
  void _drawSteppedConnection(
    ...
    if (layout == TreeLayout.bidirectional ||
        direction == TreeDirection.vertical) {
      // Use a fixed middle Y position to ensure all arrows converge to the same point
      // This makes all child-to-parent connections meet at the same horizontal line
      final middleY = layout == TreeLayout.bidirectional
          ? (start.dy > end.dy
              ? end.dy +
                  (start.dy - end.dy) * 0.5 // Teachers above (coming down)
              : start.dy +
                  (end.dy - start.dy) * 0.5) // Students below (going down)
          : start.dy + (end.dy - start.dy) * 0.5;
      path.moveTo(start.dx, start.dy);
